...
def ensure_flux():
 print('KAGGLE_ENGINE=flux1-schnell-nf4-yield-router-v9-multibranch-semantic-safe',flush=True)
...
branch_report=incoming/'branch-search-report.json'
if branch_report.is_file():shutil.copy2(branch_report,OUT/'branch-search-report.json')
...
(OUT/'generated-targets.json').write_text(json.dumps({'count':len(targets),'engine':'FLUX.1-schnell NF4 yield-routed batch v9 multibranch-semantic-safe','lane':lane,'seed':SEED,'backlog':q,'targets':targets},indent=2),encoding='utf-8');print(f'KAGGLE_EXPORT_COUNT={len(fresh)}',flush=True);print('KAGGLE_OUTPUT_ONLY=1',flush=True)