...
def ensure_flux():
 print('KAGGLE_ENGINE=flux1-schnell-nf4-yield-router-v6-retry-normalized',flush=True)
...
if q['BLD']>=5:
 lane='BUILDING_FAMILIES';effective=max(7,min(COUNT,28));factory=REPO/'tools/sprites/kaggle_building_family_factory.py';cmd=['python','-u',str(factory),'--count',str(effective),'--seed',str(SEED)]
elif q['STATIC']:
...
(OUT/'generated-targets.json').write_text(json.dumps({'count':len(targets),'engine':'FLUX.1-schnell NF4 yield-routed batch v6 retry-normalized','lane':lane,'seed':SEED,'backlog':q,'targets':targets},indent=2),encoding='utf-8');print(f'KAGGLE_EXPORT_COUNT={len(fresh)}',flush=True);print('KAGGLE_OUTPUT_ONLY=1',flush=True)