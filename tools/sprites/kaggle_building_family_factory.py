...
print('KAGGLE_STARTUP=building-family-flux-v10-coherence-safe',flush=True)
import torch
...
PRIORITY=(5,6,4,7,8,9,10,12,13,11,3,0,1,2); STRENGTH={1:.36,2:.44,3:.52,4:.60,5:.68,6:.72}
def rows():
...
def prompts(i):
 ...
 d=f"Premium mobile strategy building master. Fixed family DNA: {DNA[i['family']]}. {DELTA[i['tier']]}. Preserve facade, production core, roof orientation, camera center and structural anchors; every addition physically attached. 34 degree three-quarter orthographic camera, bottom-center grounding, upper-left key, cool fill, restrained cyan/warm emissives. Uniform achromatic background touching every edge. No gradient, scenery or floor card. No detached cables, long stray lines, floating debris, particles or disconnected props. No people or vehicles. No readable text, letters, numbers, labels, callouts, arrows, diagrams, annotations, signage, logos, watermark or UI."
 return s,d
def load_encode():
...
def finish(raw):
 ...
 if len(cs)>1 and len(cs[0])/sum(map(len,cs))<.93:raise RuntimeError('detached structure')
