...
print('KAGGLE_STARTUP=building-family-flux-v12-retry-normalized', flush=True)
import torch
...
def prompts(i):
    base=(f"AAA premium mobile 2.5D strategy building, family {i['family']:02d}, tier {i['tier']}. "
          f"Exactly one connected building. Fixed family DNA: {DNA[i['family']]}. {DELTA[i['tier']]}. ")
    composition=("Same building upgraded in place; preserve facade, production core, roof orientation and structural anchors. "
                 "Every new part must be physically attached. 34 degree three-quarter orthographic camera, centered composition, "
                 "bottom-center grounding, upper-left key light, cool fill, restrained cyan or warm emissives. ")
    isolation=("CUTOUT PRODUCT RENDER on a perfectly flat uniform neutral medium-gray studio background touching every image edge. "
               "No floor, no platform card, no terrain slab, no horizon, no scenery, no environment, no vignette, no gradient, no cast ground plane. ")
    negatives=("No people, workers, vehicles, roads, arrows, labels, signs, readable text, pseudo-text, letters, numbers, logos, watermark, UI, "
               "detached props, floating debris, particles, loose cables or disconnected components. Clean continuous materials; no accidental holes. ")
    ...
    return base+isolation, base+composition+isolation+negatives+tier0
...
def isolate(im):
    ...
    if sd>10 or chroma>18: raise RuntimeError(f'bad border sd={sd:.1f} chroma={chroma}')
    px=rgb.load();mask=Image.new('L',(w,h));mp=mask.load()
    for y in range(h):
        for x in range(w):
            p=px[x,y];d=((p[0]-bg[0])**2+(p[1]-bg[1])**2+(p[2]-bg[2])**2)**.5
            mp[x,y]=0 if d<8 else 255 if d>24 else int((d-8)*255/16)
...
def internal_hole_score(alpha):
    ...
    while q:
        ...
        for n in ((x-1,y),(x+1,y),(x,y-1),(x,y+1)):
            u,v=n
            if 0<=u<96 and 0<=v<96 and n not in seen: q.append(n)
    ...
    while inner:
        ...
        holes.append(len(comp))
    return max(holes,default=0)/(96*96)
...
def finish(raw,tier):
    ...
    if hole>.045: raise RuntimeError(f'internal-hole-score={hole:.3f}')
    ...
    if cov<lo or cov>.60: raise RuntimeError(f'coverage {cov:.1%}')
    pad=int(side*.08)
...
def bbox_metrics(im):
    ...
    return bb[2]-bb[0],bb[3]-bb[1],(bb[0]+bb[2])/2,(bb[1]+bb[3])/2

def family_qa(recs):
    if len(recs)<2:return True,'single'
    adj=[iou(recs[n-1][1],recs[n][1]) for n in range(1,len(recs))]
    if min(adj)<.34:return False,f'identity-iou={min(adj):.2f}'
    cov=[r[2] for r in recs]
    drops=sum(1 for a,b in zip(cov,cov[1:]) if b<a*.92)
    if drops>1:return False,f'nonmonotonic-coverage={cov}'
    if cov[-1]<cov[0]*1.42:return False,f'insufficient-growth={cov[0]:.3f}->{cov[-1]:.3f}'
    boxes=[bbox_metrics(r[1]) for r in recs]
    cx0=boxes[0][2]; max_center=max(abs(b[2]-cx0) for b in boxes)
    if max_center>5.5:return False,f'horizontal-drift={max_center:.1f}'
    if iou(recs[0][1],recs[-1][1])>.90:return False,'insufficient-evolution'
    return True,f'min-adj-iou={min(adj):.2f} growth={cov[-1]/cov[0]:.2f}x drift={max_center:.1f}'

def render_with_retries(i,previous,pe,ppe,base,img,seed):
    errors=[]
    for attempt in range(RETRIES.get(i['tier'],2)):
        ...
        try:
            ...
            print(f"KAGGLE_RETRY={i['id']} attempt={attempt+1} reason={type(e).__name__}: {e}",flush=True)
        finally:
