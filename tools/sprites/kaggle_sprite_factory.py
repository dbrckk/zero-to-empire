...
def concrete_subject(i):
 ...
 if i['kind']=='VEH':
  exact={
   ...
   'VEH-16':'one compact two-passenger prestige executive anti-gravity coupe, low sleek teardrop capsule body, approximately twice as long as tall, panoramic dark glass canopy, completely blank unbranded featureless nose, smooth uninterrupted rounded belly with absolutely nothing projecting or detached beneath it, a very thin cyan levitation light seam embedded flush inside the lower hull surface, visibly hovering high with a large open empty air gap beneath the complete hull, pearl white metal and glossy black premium finish, no wheels, no wheel arches, no legs, no feet, no struts, no landing gear, no support hardware, no skid, no rail, no blade, no bar, no pod underneath',
...
def prompt_for(i):
 ...
 if i['id']=='VEH-16':
  return f"AAA mobile strategy game sprite of {s}. ONE small personal luxury hover coupe only, fully visible, three-quarter 34 degree view, floating high in empty space, isolated on pure black with generous margin. Low aerodynamic capsule silhouette, not a bus, van, shuttle or sedan. The underside must be one continuous smooth closed hull; the cyan levitation light must be painted/embedded flush into that hull and must NOT form any separate object. Large clean empty black air gap below every part of the vehicle. Upper-left key light, cool fill. No road, floor, pedestal, detached underside object, glowing blade, skid, rail, bar, pod, text, symbol, badge, logo, UI or other object."
