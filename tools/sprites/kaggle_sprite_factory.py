...
def prompt_for(item):
    ...
    return (
        ...
        "Upper-left key light, cool fill, restrained amber and cyan emissive accents, premium physically plausible materials. "
        "ONE isolated object only, centered on pure solid black background, large empty margin, no surrounding scene. "
        "No UI, no border, no frame, no watermark, no logo, no letters, no readable text, no numbers, no brand signage, no collage, no multiple objects."
    )


def load_pipe():
    token = os.getenv("HF_TOKEN") or None
    print(f"Loading {LIGHTNING_CKPT} on CUDA...")
...
def border_background(im: Image.Image):
    ...
    for x in range(0, w, stride):
        pts.append(rgb.getpixel((x, 0))); pts.append(rgb.getpixel((x, h - 1)))
    for y in range(0, h, stride):
        pts.append(rgb.getpixel((0, y))); pts.append(rgb.getpixel((w - 1, y)))
    pts.sort(key=lambda p: sum(p))
    sample = pts[: max(16, len(pts) // 3)]
...
def isolate(im: Image.Image):
    ...
    for y in range(h):
        for x in range(w):
            ...
            dp[x, y] = min(255, int(d * 3.0))
    mask = dist.point(lambda p: 0 if p < 20 else (255 if p > 58 else int((p-20) * 255 / 38)))
    # Remove only background connected to the canvas edge; preserve enclosed dark materials.
    ...
    while q:
        ...
        for nx, ny in ((x-1,y),(x+1,y),(x,y-1),(x,y+1)):
            if 0 <= nx < w and 0 <= ny < h and (nx,ny) not in seen and mp[nx,ny] < 180:
...
def normalize(master: Image.Image, side: int):
    ...
    x = (side-crop.width)//2
    bottom = int(side*.08)
    y = max(int(side*.08), side-bottom-crop.height)
...
def validate(im: Image.Image):
    ...
    if visible < .04 or visible > .70:
        raise RuntimeError(f"invalid alpha coverage {visible:.1%}")
    pad = int(im.width*.04)
    ...
    if any(e.getbbox() for e in edges):
...
def main():
    ...
    ap.add_argument("--kind", choices=["ALL", "PRP", "VEH", "CORE", "BLD"], default="ALL")
    ap.add_argument("--count", type=int, default=60)
    ap.add_argument("--seed", type=int, default=4242)
    args = ap.parse_args()
    items = list(manifest_rows())
    if args.kind != "ALL":
        items = [x for x in items if x["kind"] == args.kind]
    items.sort(key=lambda x: (PRIORITY[x["kind"]], x["order"]))
    items = items[:max(1, args.count)]
    if not items:
        print("No pending static manifest assets.")
        return
    pipe = load_pipe()
    ...
    for index, item in enumerate(items, 1):
        ...
        gen = torch.Generator(device="cuda").manual_seed(args.seed + index)
        try:
            image = pipe(
                prompt_for(item),
            ...
            final = normalize(isolate(image), TARGET_SIDE[item["kind"]])
            coverage = validate(final)
            out = INCOMING / f"{item['stem']}.png"
            final.save(out, "PNG", optimize=True)
            ok += 1
            print(f"KAGGLE_VALIDATED={out.relative_to(ROOT)} coverage={coverage:.1%}")
        except Exception as exc:
            rejected += 1
            print(f"KAGGLE_REJECTED={item['id']} reason={exc}")
        finally:
            torch.cuda.empty_cache()
    print(f"KAGGLE_BATCH_SUCCESS={ok} KAGGLE_BATCH_REJECTED={rejected} KAGGLE_BATCH_ATTEMPTED={len(items)}")
